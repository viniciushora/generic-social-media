...
class Post {
    ...
    static async findByIdAndEdit(postId, userId, text){
        let query = { _id: postId, createdBy: userId };
        let newvalues = { $set: { textContent: text } };
        return await PostMongo.updateOne(query, newvalues, { upsert: false })
    }
}
