...
const formatPost = (post) => {
    ...
    if (loggedUser && loggedUser.textContent == post.author) {
        fpost.innerHTML += `<span class="fa fa-trash" aria-hidden="true"
                            ...
                            data-postid="${post._id}" id="lixeira-${post._id}"></span>
                            <span class="fa fa-edit" aria-hidden="true"
                            style="margin-left:1em"
                            data-postid="${post._id}" id="edit-${post._id}"></span>`
        const buttonDelete = fpost.querySelector("#lixeira-" + post._id.toString())
        const buttonEdit = fpost.querySelector("#edit-" + post._id.toString())

        buttonDeleteSetBehavior(buttonDelete)
        buttonEditSetBehavior(buttonEdit)
    }
    return fpost
}
